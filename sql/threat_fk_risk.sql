-- Forensic integrity: prevent parent deletes from erasing observation/link history.
-- Prefer RESTRICT / NO ACTION over CASCADE.

-- threat_observations already uses ON DELETE RESTRICT (see create table).
-- Recreate threat_links FKs without CASCADE.

alter table public.threat_links
  drop constraint if exists threat_links_left_hash_fkey;

alter table public.threat_links
  drop constraint if exists threat_links_right_hash_fkey;

alter table public.threat_links
  add constraint threat_links_left_hash_fkey
  foreign key (left_hash)
  references public.threat_records (content_hash)
  on delete restrict;

alter table public.threat_links
  add constraint threat_links_right_hash_fkey
  foreign key (right_hash)
  references public.threat_records (content_hash)
  on delete restrict;

comment on constraint threat_links_left_hash_fkey on public.threat_links is
  'RESTRICT: deleting a threat_record must fail while links exist (forensic integrity).';

comment on constraint threat_links_right_hash_fkey on public.threat_links is
  'RESTRICT: deleting a threat_record must fail while links exist (forensic integrity).';

-- Risk scoring columns on current consolidated records
...
alter table public.threat_records
  add column if not exists risk_level text
    check (
      risk_level is null
      or risk_level in ('critical', 'high', 'medium', 'low', 'informational')
    );

alter table public.threat_records
  add column if not exists risk_factors jsonb not null default '[]'::jsonb;

comment on column public.threat_records.risk_score is
  'Deterministic impact/urgency score for the indicator. Separate from confidence.';

-- Immutable risk snapshot on observations
...
comment on column public.threat_observations.risk_score is
  'Risk at observation time (immutable). Later sightings may refresh threat_records.risk_* only.';

-- Extend upsert_threat_record to persist risk fields
...
begin
  insert into public.threat_observations (
    ...
    payload_hash, metadata, risk_score, risk_level, risk_factors, created_at
  ) values (
